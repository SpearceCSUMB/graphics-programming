...
var Sphere = function(center, radius) {
  ...
  this.raycast = function(ray) {
    ...
    var alpha = ray.direction.dot(ray.direction);
    var bravo = (ray.direction.dot((ray.origin.subtract(this.center)))) * 2;
    var charlie = (ray.origin.subtract(this.center)).dot((ray.origin.subtract(this.center))) - Math.pow(this.radius, 2);

    var discriminant = (Math.pow(bravo, 2) - (4*alpha*charlie));
    var numerator = (-bravo + Math.sqrt(discriminant));
    var denominator = (2*alpha);
